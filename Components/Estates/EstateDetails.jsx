    ...
    return (
      ...
      >
       {estateDetails.preview_img.map((estate,idx)=><SwiperSlide key={idx}><img src={estate} className="h-[200px] md:h-[300px] lg:h-[400px] w-full" alt="" /></SwiperSlide>)}
            ...
            {estateDetails.facilities.map(facility=>(<p className="font-semibold font-serif"data-aos="fade-up" data-aos-duration="1400">{facility}</p>))}
            ...
            <p className='flex gap-2 items-center'><FaChartArea className='text-3xl text-yellow-600' /> <span className='text-yellow-600 font-bold'>{estateDetails.area}</span></p>
